...
function getTeams() {
	$.get("http://172.16.220.241:3000/api/Team", function (data) {
		var teams_global = data.map(team => {
			return {
				id: team.teamId,
		...
		build_user_panels(teams_global);
		// console.log(teams_global);
		getCampaigns(teams_global);
	});
}

function getCampaigns(teams) {
	$.get("http://172.16.220.241:3000/api/Campaign", function (data) {
		// console.log(data);
		var campaigns = data.map(campaign => {
			const owner = campaign.owner.replace('resource:org.cpdao.network.Team#', '');
			const ownerObj = teams.filter(x => x.id == owner)[0];
			
			const campaignAttributes = Object.keys(campaign).filter(key => key !== "campaignId" && !isNaN(campaign[key]) && campaign[key] > 0);
		...
		const obj = {marbles: campaigns};
		populate_users_marbles(obj);
	});
...
$(document).on('ready', function () {
	...
	function addTeamAttributes(attributes) {
		const divWrapper = $('#marbleTeamAttributes');
		divWrapper.html('');
		var cList = $('<ul/>');
		...
		cList.appendTo(divWrapper);
	}

	function auditMarble(that, open) {
		var marble_id = $(that).attr('id');
		var campaignName = $(that).text();
		let attributes = Array.from($(that)[0].attributes);

		attributes = attributes.filter(el => el.name.indexOf('attr_') > -1).map(el => el.name.replace('attr_',''));
		console.log(attributes);

		addTeamAttributes(attributes);


		$('#auditLegend').html($(that).attr('username'));
		...
		if (open || $('#auditContentWrap').is(':visible')) {
			...
			$('#marbleTeamName').html(campaignName);
