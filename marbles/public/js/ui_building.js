...
function build_marble(marble) {
	...
	marble.campaignAttributes.forEach(attr => html+= ' attr_' + attr + '="true" ');
	const spanText = marble.name.length > 10 ? marble.name.slice(0,10) + '...' : marble.name;
	html += ' username="' + marble.owner.username + '" company="' + marble.owner.company + '" owner_id="' + marble.owner.id + '">' + spanText + '</span>';
...
function size_user_name(name) {
	var style = '';
	if (name.length >= 8) style = 'font-size: 22px;';
	if (name.length >= 10) style = 'font-size: 18px;';
	if (name.length >= 15) style = 'font-size: 15px;';
